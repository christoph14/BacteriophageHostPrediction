...
def dna_features(dna_sequences):
    ...
    for item in dna_sequences:
        ...
        # GC content
        GC_content.append(gc_fraction(item, ambiguous="ignore") * 100)
...
def protein_features(protein_sequences):
    ...
    for item in protein_sequences:
        ...
        frac_polar.append(
            (item.count('Q') + item.count('N') + item.count('H') + item.count('S') + item.count('T') + item.count('Y')
...
def define_groups(alignment_scores, threshold):
    """Compute the group for each instance based on their similarity.

    Parameters
    ----------
    alignment_scores : array-like of shape (n_instances, n_instances)
        Symmetric matrix containing the alignment scores of all instances.
    threshold : float
        The threshold defining which instances belong in the same group.

    Returns
    -------
    groups_array : np.ndarray of shape (n_instances)
        Array with the group of each instance.
    """
    # Get all tuples of similar instances
    similar_instances = np.where(alignment_scores >= threshold)
    similar_instances = np.unique(np.sort(np.array(similar_instances).T, 1), axis=0)

    # Compute the groups of all instances
    groups_array = np.arange(len(alignment_scores))
    for i, j in similar_instances:
        new_group = min(groups_array[i], groups_array[j])
        old_group = max(groups_array[i], groups_array[j])
        update_idx = np.where(groups_array == old_group)
        groups_array[update_idx] = new_group
    return groups_array
...
def NestedGroupKFold(model, X, y, parameter_grid, groups, class_weights, scorer=make_scorer(accuracy_score),
    ...
    # Use only one job for logistic regression to avoid ConvergenceWarnings
    n_jobs = 1 if model.steps[1][0] == 'logisticregression' else -1

    # define outer loop
    for loop, (train_outer, test_outer) in enumerate(outer_cv.split(X, y, groups)):
        _, counts = np.unique(y[test_outer], return_counts=True)
        # print("Class distribution:", np.round(counts * 100 / sum(counts), 2))
        ...
        tuned_model = GridSearchCV(model, cv=inner_cv, param_grid=parameter_grid, scoring=scorer, n_jobs=n_jobs)
